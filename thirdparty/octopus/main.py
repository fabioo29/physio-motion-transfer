...
from keras.layers import Input, Flatten, Dense, Lambda, Conv2D, MaxPool2D, Average, Concatenate, Add, Reshape, initializers, activations
...
def read_segmentation(file):
    segm = cv2.resize(cv2.imread(file)[:, :, ::-1], (1080, 1080))
...
def main(weights, num, batch_file, opt_pose_steps, opt_shape_steps):
    '''
    INPUTS:
        weights: pretrained model weights location
        num: number of samples from the batch to work with
        batch_file: batch with samples location
        opt_pose_steps: steps for optimizing the model pose
        opt_shape_steps: steps for optimizing the model shape
    OUTPUTS:
        xyz.pkl
    '''
    ...
    with open(batch_file, 'rb') as f:
        ...
        for name, segm_dir, pose_dir, out_dir in reader:
            ...
            pickle_out = open("./out/frame_data.pkl","wb")
...
if __name__ == '__main__':
    main('assets/octopus_weights.hdf5', 1, 'data/batch', 5, 15)
