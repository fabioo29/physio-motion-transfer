'''
    MESA_GL_VERSION_OVERRIDE=4.1 CUDA_VISIBLE_DEVICES=0 python infer.py --configs_yml=assets/video.yml
    MESA_GL_VERSION_OVERRIDE=4.1 CUDA_VISIBLE_DEVICES=0 python infer.py --configs_yml=assets/single_image.yml
'''
...
def get_renderer(test=False,resolution = (512,512,3),part_segment=False):
    faces = pkl.load(open(os.path.join(args.smpl_model_path,'SMPL_NEUTRAL.pkl'),'rb'), encoding='latin1')['f']
...
def write_frame_data(filename, vertices):
...
class Renderer:
    ...
    def __call__(self, verts=None, cam=[0.99,0.99,-0.02,0.26], angle=None, axis=None, mesh_filename=None, color=None, tex=None, obj=None):

        obj = io.StringIO(obj)

        mat = trimesh.visual.material.SimpleMaterial(image=tex)
        mesh = trimesh.exchange.load.load(obj, file_type='obj', force='mesh')
...
class Visualizer(object):
    ...
    def visualize_renderer(self, tex, verts, images=None, reorganize_idx=None,thresh=0.2,visible_weight=0.9, scale_thresh=100):

        renders = []

        render_result = self.renderer(tex=tex, obj=verts)
        renders.append(render_result)
        renders = np.array(renders)
        
        return renders.astype(np.uint8)

    def visulize_result_onorg(self, data, tex): 
        
        out_list = []

        background = np.array(np.zeros((1080,1080,3), np.uint8)) # all white

        for obj in tqdm(data):
            rendered_imgs = self.visualize_renderer(tex, obj, images=background)
            out_list.append(rendered_imgs[:,:,::-1])
...
class Demo(Base1):
    ...
    def _prepare_modules_(self):
        ...
        self.vis_size = [1080,1080,3] #[1920,1080]
        ...
        if self.save_mesh:
            self.smpl_faces = pkl.load(open(os.path.join(args.smpl_model_path,'SMPL_NEUTRAL.pkl'),'rb'), encoding='latin1')['f']
        #print('Initialization finished!')

    def run(self, data, texture):
        
        with torch.no_grad():
            if not os.path.exists('output'):
                os.makedirs('output')  
            
            return self.visualizer.visulize_result_onorg(data, texture)[:-1]
    ...
    def process_video(self, video_file_path=None):
        video = [os.path.join(video_file_path, file) for file in os.listdir(video_file_path) if file.endswith(".mp4")]
        capture = OpenCVCapture(video[0])
...
class ResultParser(nn.Module):
    ...
    def val_forward(self, outputs, meta_data, cfg):
        ...
        #print("Betas location", str(outputs['params']['betas'].shape))
        #print("Pose location", str(outputs['poses'].shape)) 
